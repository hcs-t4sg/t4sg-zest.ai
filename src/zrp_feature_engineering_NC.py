...
class ZRPFeatureEngineeringNC():
    ...
    def __init__(self):
        ...
        self.oe = OrdinalEncoder(handle_unknown='ignore')
...
#       The end codes are the same as for FL model but mapping differs  
        census_code_mapping =  {'I': 'AI', 'P': 'API', 'A':'API', 'B': 'Black', 'H': 'Hispanic', 'W': 'White', 'O': 'Other', 'M': 'Multi', 'U': 'Unknown'}
    ...
    def transform(self, X):
        ...
        X[['Gender']] = X[['Gender']].replace(to_replace='nan', value=' ')
        ...
        for i in range(self.n_classes):
            print(self.le[i].transform(X[self.label_encoded_columns]), file = sys.stdout)
